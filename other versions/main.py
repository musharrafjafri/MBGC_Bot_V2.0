...
def desire_date_fun():
    ...
    return desire_date, is_date_avail
...
def is_course_available_fun():
    ...
    try:
        WebDriverWait(driver, 1).until(
            EC.presence_of_all_elements_located((By.XPATH, '//*[@id="AvailCourse"]/a'))
        )
...
def main_fun():
    ...
    desire_date, date_flag = desire_date_fun()  # getting desire date and date flag for date availability in current month.
    sleep(1)
    if date_flag:  # If date is available in current month than just click on that desire date.
        desire_date.click()
    else:  # If date is not in current month than run the next month function.
        next_month_fun()
    sleep(1)
    is_course_available = is_course_available_fun()  # Getting flag if courses or desire course is available or not.
    while is_course_available == False:  # Run this loop while courses appear on page or our specific (Day Golf (18 Holes) course.
        ...
        sleep(1)
        desire_date, date_flag = desire_date_fun()  # Agian getting desire date and date flag for date availability in current month.
        if date_flag:  # If date is available in current month than just click on that desire date.
            desire_date.click()
        else:  # If date is not in current month than run the next month function.
            next_month_fun()
    ...
    if desire_time_id != []:  # If desire time ids is not Null than move ahead
        ...
        incompl_flag = is_time_hold_fun()  # Check if desire time is not on hold.
        while incompl_flag:  # if desire time is on hold than run this loop until find the timeslot unhold.
            print("TRYING TO FIND TIMSLOT WHICH IS NOT BOOKED, PLEASE WAIT...")  # print this line until timeslot unhold.
            driver.refresh()  # refresh the page
            sleep(0.5)  # wait 1 second to load the page.
